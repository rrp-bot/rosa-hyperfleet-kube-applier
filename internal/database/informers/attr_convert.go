...
// (streamtypes.AttributeValue) to the equivalent DynamoDB API attribute value
// map (dbtypes.AttributeValue). The two SDKs define separate but structurally
// identical types; this function performs the necessary recursive conversion so
// that the database package's item-to-desire helpers can be used on stream
// images.
...
func streamAttrToDB(av streamtypes.AttributeValue) dbtypes.AttributeValue {
	...
	default:
		// Unknown type — return NULL to avoid panics.
